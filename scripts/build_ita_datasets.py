...
MC_COLUMNS = [
    "alternative_id", "question_id", "year", "exam_phase", "question_number",
    "subject", "question_text", "alternative", "alternative_text", "has_image",
    "ocr_used", "needs_review", "source_pdf", "page_start", "page_end",
]
ESSAY_COLUMNS = [
    "question_id", "year", "exam_phase", "question_number", "subject",
    "question_text", "has_image", "ocr_used", "needs_review", "source_pdf",
    "page_start", "page_end",
]
IMAGE_COLUMNS = [
    "image_id", "question_id", "image_path", "page", "width_px", "height_px",
]
...
def infer_subject_from_filename(pdf: Path) -> str | None:
    ...
    for token, subject in (
        ("FISICA", "fisica"), ("MATEMATICA", "matematica"),
        ("QUIMICA", "quimica"), ("INGLES", "ingles"),
        ("PORTUGUES", "portugues"),
    ):
...
def native_text_is_suspicious(lines: list[TextLine]) -> bool:
    ...
    if not text:
        return True
    if any(token in text for token in ("4XHVW", "&RQVWDQWH", "SURSRVLo", "TXHVWmR")):
        return True
...
def extract_lines(page: fitz.Page) -> tuple[list[TextLine], bool]:
    ...
    ocr = payload_to_lines(page, page.get_text("dict", textpage=textpage, sort=True))
...
def find_bare_question_one(
    ordered: list[QuestionStart],
    all_lines: list[list[TextLine]],
    year: int,
    fixed_subject: str | None,
) -> QuestionStart | None:
    if fixed_subject is None or not ordered or ordered[0].number != 2:
        return None
    first = ordered[0]
    matches: list[QuestionStart] = []
    for page_index, lines in enumerate(all_lines):
        ...
        for line in lines:
            ...
            if match and int(match.group(1)) == 1:
                matches.append(QuestionStart(year, 1, fixed_subject, page_index, line.y0))
    return matches[-1] if matches else None


def select_sequential_questions(
    ...
    bare_one = find_bare_question_one(ordered, all_lines, year, fixed_subject)
    if bare_one is not None:
        ordered.append(bare_one)
        ordered.sort(key=lambda q: (q.page_index, q.y0, q.number))

    q1_indices = [i for i, item in enumerate(ordered) if item.number == 1]
    seed_index = q1_indices[0] if q1_indices else 0
...
def discover_questions(
    ...
    for page in doc:
        ...
        for index, line in enumerate(lines):
            ...
            candidates.append(
                QuestionStart(
                    year, parse_question_number(match.group(1)), current_subject,
                    page.number, line.y0,
                )
            )

    starts = select_sequential_questions(candidates, all_lines, year, fixed_subject)
...
def make_question_id(kind: QuestionKind, year: int, phase: str, subject: str, number: int) -> str:
...
def parse_pdf(
    ...
    with fitz.open(pdf) as doc:
        starts, stops, all_lines, page_ocr = discover_questions(doc, year, fixed_subject)
        if not starts:
        ...
        for index, start in enumerate(starts):
            ...
            if kind == "multiple_choice":
                ...
                images = render_question_images(
                    doc, qid, year, segments, MC_IMAGE_ROOT, scale, needs_review
                )
                for label in "ABCDE":
                    mc_rows.append({
                ...
                images = render_question_images(
                    doc, qid, year, segments, ESSAY_IMAGE_ROOT, scale, needs_review
                )
                essay_rows.append({
                    "question_id": qid,
                    "year": year,
        ...
        for sheet_name, frame in (("questions", questions), ("images", images)):
            ...
            for column_index, column in enumerate(frame.columns):
                ...
                if column in {"question_text", "alternative_text"}:
                    ...
                    worksheet.set_column(column_index, column_index, max(12, min(22, len(column) + 3)))
...
def validate_historical_counts(mc: pd.DataFrame, essay: pd.DataFrame) -> None:
    ...
    for year in range(LEGACY_FIRST_YEAR, LEGACY_LAST_YEAR + 1):
        year_mc = mcq[(mcq["year"] == year) & (mcq["exam_phase"] == "single_phase")]
        year_essay = essay[(essay["year"] == year) & (essay["exam_phase"] == "single_phase")]
        mc_counts = year_mc.groupby("subject").size().to_dict()
        essay_counts = year_essay.groupby("subject").size().to_dict()
        expected_mc = {subject: 20 for subject in (*sorted(SCIENCE_SUBJECTS), *sorted(LANGUAGE_SUBJECTS))}
        expected_essay = {subject: 10 for subject in sorted(SCIENCE_SUBJECTS)}
        if mc_counts != expected_mc:
            raise ValueError(f"{year}: legacy MC counts {mc_counts}, expected {expected_mc}")
        if essay_counts != expected_essay:
            raise ValueError(f"{year}: legacy essay counts {essay_counts}, expected {expected_essay}")

    expected_phase1 = {
        2019: 60, 2020: 70, 2021: 70, 2022: 70,
        2023: 60, 2024: 60, 2025: 48, 2026: 48,
    }
    for year, expected in expected_phase1.items():
        count = len(mcq[(mcq["year"] == year) & (mcq["exam_phase"] == "phase_1")])
        if count != expected:
            raise ValueError(f"{year}: phase-1 MC count {count}, expected {expected}")
        ...
        expected_science = {subject: 10 for subject in sorted(SCIENCE_SUBJECTS)}
        if subject_counts != expected_science:
            raise ValueError(
                f"{year}: phase-2 science essay counts {subject_counts}, expected {expected_science}"
            )
    ...
    for year in (2025, 2026):
        ...
        if len(port_mc) != 15 or len(port_essay) != 0:
            raise ValueError(
                f"{year}: expected 15 objective Portuguese questions and 0 essay questions; "
...
def validate_outputs(
    ...
    if mc["alternative_id"].duplicated().any() or essay["question_id"].duplicated().any():
        raise ValueError("Question identifiers are not unique")
    ...
    if not (mc.groupby("question_id")["alternative"].nunique() == 5).all():
        raise ValueError("At least one multiple-choice question does not have five alternatives")
...
def clean_outputs() -> None:
    for path in (
        MC_IMAGE_ROOT, ESSAY_IMAGE_ROOT,
        DATA_ROOT / "ita_phase_1_images",
    ):
        if path.is_dir():
            shutil.rmtree(path)
    for path in (
        MC_DATASET, MC_IMAGE_DATASET, MC_EXCEL,
        ESSAY_DATASET, ESSAY_IMAGE_DATASET, ESSAY_EXCEL,
...
def build(input_root: Path, scale: float) -> None:
    ...
    mc_rows.sort(key=lambda r: (r["year"], r["exam_phase"], r["subject"], r["question_number"], r["alternative"]))
    essay_rows.sort(key=lambda r: (r["year"], r["exam_phase"], r["subject"], r["question_number"]))
    ...
    essay_image_df = write_parquet(essay_images, IMAGE_COLUMNS, ESSAY_IMAGE_DATASET)
